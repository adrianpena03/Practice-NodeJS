...
server.listen(3000, () => {
    ...
    db.connect((err) => {
        ...
        db.query('CREATE DATABASE IF NOT EXISTS sailingadventure_db', (err) => {
            if (err) {
                console.error('Error creating database: ' + err.message);
                return;
            }
            console.log('Database sailingadventure_db created or already exists!');

            // Use the database after creating it
            db.changeUser({ database: 'sailingadventure_db' }, (err) => {
                if (err) {
                    console.error('Error switching to sailingadventure_db database: ' + err.message);
                    return;
                }
                console.log('Using sailingadventure_db database.');

                // Import CRUD operations here after creating the database
